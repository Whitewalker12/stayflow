...
export function BookingCalendar({
  ...
  function getBlock(booking: BookingWithGuest, roomId: string) {
    ...
    if (booking.status === 'cancelled' || booking.status === 'no_show') {
      // Still show cancelled/no-show but styled differently
    }
    ...
    // No overlap
    if (checkOut <= startDate || checkIn >= rangeEnd) return null

    // Clamp to visible range
    const visStart = checkIn < startDate ? startDate : checkIn
    const visEnd = checkOut > rangeEnd ? rangeEnd : checkOut
    ...
    // Clipped on left/right
    const clippedLeft = checkIn < startDate
    const clippedRight = checkOut > rangeEnd

    return { colStart, colSpan, clippedLeft, clippedRight }
  }
